...
//returns the acc, and do things to the value.
//}
const addValues = (arr) => {
...
const returnNames = (arr) => {
  let results = arr.reduce((acc, value) => {
    acc.push(value.name);
    return acc;
  }, []);
  return results;
...
const calculateAverage = (arr) => {
  let avgValues = arr.reduce((acc, val) => {
    acc.count++;
    acc.sum += val;
    return acc;
  }, {count: 0, sum: 0});
  return (avgValues.sum / avgValues.count);
};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 9 - Stretch Goal

Write a function named countPrimeNumbers that, given an array elements as input, uses reduce to count the number 
of elements that are prime numbers.
...
const extractStat = (statName, arr) => {
  return arr.reduce((acc, val) => {
    console.log(val.stat.name, statName);
...
const extractChildren = (arr) => {
  let filteredArr = arr.filter((val, idx) => val.name.includes('a'));
